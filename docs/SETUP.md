# Getting Started with bot_army_discovery

This guide walks you through setting up Discovery Bot for local development.
...
DATABASE_NAME=bot_army_discovery_dev
```
...
Discovery Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_discovery/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_discovery/skills/example.ex lib/bot_army_discovery/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyDiscovery.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_discovery.command.classify"]
...
```elixir
# In lib/bot_army_discovery/application.ex or similar

defmodule BotArmyDiscovery.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyDiscovery.Skills.Classify,
      BotArmyDiscovery.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_discovery/skills/my_skill.ex
...
releases: [
  discovery_bot: [
    applications: [bot_army_discovery: :permanent]
  ]
]
```

This creates a release named `discovery_bot` that is deployed to `/opt/ergon/releases/discovery_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/discovery_bot/`
