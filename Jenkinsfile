pipeline {
  ...
  environment {
    BOT_NAME = 'discovery_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-discovery"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/discovery_bot/bin/discovery_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyDiscovery.Release.create()'"
          $RELEASE_BIN eval 'BotArmyDiscovery.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyDiscovery.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyDiscovery.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_discovery/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_discovery/releases/start_erl.data)
        fi
